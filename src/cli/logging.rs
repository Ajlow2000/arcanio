...
pub fn setup_logging(logging_config: &LoggingConfig) -> Result<()> {
    let console_level = match logging_config.level.as_str() {
        "off" => LevelFilter::OFF,
    ...
    let console_layer = tracing_subscriber::fmt::layer()
        ...
        .with_file(logging_config.show_file)                // Display source code file paths
        .with_line_number(logging_config.show_line_numbers) // Display source code line numbers
        .with_thread_ids(logging_config.show_thread_ids)    // Display the thread ID an event was recorded on
        .with_target(logging_config.show_target)            // Display the event's target (module path)
    ...
    if logging_config.file_enabled {
        let file_level = match logging_config.file_level.as_str() {
            "off" => LevelFilter::OFF,
            level_str => LevelFilter::from_level(
                Level::from_str(level_str).map_err(|_| {
                    Error::LoggingSetupError
        ...
        let rotation = match logging_config.file_rotation.as_str() {
        ...
        let file_path = std::path::Path::new(&logging_config.file_path);
        ...
        let file_layer = tracing_subscriber::fmt::layer()
            ...
            .with_file(logging_config.show_file)
